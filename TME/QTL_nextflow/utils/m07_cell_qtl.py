...
pre = args.prefix
bim_fn = f'{pre}.germ_dedup.bim'
qtl_fn = f'{pre}.qtl.txt.gz'
...
def add_snp_info(qtl_fn, bim_fn):
    ...
    for qtl_df in pd.read_csv(qtl_fn,sep='\t',header=0,compression='gzip',chunksize=1e6):
        ...
        pre_col = ['chr','pos']
        columns = ['chr','pos'] + [c for c in merge_df.columns if c not in pre_col]
        os.makedirs('cell',exist_ok=True)
        for cell, df in merge_df.groupby('gene'):
            cell = format_cell_name(cell)
            out_fn = f'cell/{pre}.{cell}.txt.gz'
...
qtl_fns = [f for f in glob.glob(f'cell/*.txt.gz') 
